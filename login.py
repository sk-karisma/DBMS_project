...
from backend import db_api  # Import the database API file
...
def patient_interface():
    ...
    #     if bills:
    #         for b in bills:
    #             st.markdown(f"**Bill ID:** {b['B_ID']} | **Amount:** ₹{b['Amount']:.2f}")
    #     else:
        ...
        if bills:
            ...
            for bill in bills:
                ...
                if bill['Paid_Amount'] >= bill['Amount']:
                    ...
                    bill_data["Status"].append(f"⚠️ {remaining:.2f} Due")
            ...
            for i, bill in enumerate(bills):
                with st.expander(f"Bill #{bill['B_ID']} Details"):
                    ...
                    # Add download button (this is a placeholder - in a real app you'd generate a PDF)
                    st.download_button(
        ...
        with st.form("add_prescription_form"):
            ...
            if submitted:
                if patient_id and doctor_id and treatment:
                    # We need to get the admin ID from the session
                    # For simplicity, we'll use a placeholder value of 1, 
                    # but in a real app you'd get this from the logged-in admin
                    # placeholder - you should get this from session
